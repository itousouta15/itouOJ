...
android/scripts/        # icon generation script (generate-icons.mjs, sharp converts itouOJ.svg to per-density mipmaps)
...
Release signing uses `android/keystore.properties` (not committed); `assembleDebug` works without it. Icons are generated by `android/scripts/generate-icons.mjs`, which uses sharp to convert `itouOJ.svg` into per-density launcher / round / adaptive foreground icons; re-run the script after changing the LOGO.
